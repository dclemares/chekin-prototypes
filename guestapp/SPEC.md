...
> | `flow-travel-guide-mobile.html`, `flow-travel-guide-desktop.html`, `travel-guide.css`, `travel-guide.js` | §35 AI Travel Guide |
...
· **Prototypes:** `flow-travel-guide-mobile.html` (mobile), `flow-travel-guide-desktop.html`
...
| `tg-q` | `tgd-q1`..`tgd-q6` | preference questions (`?step=questions`) — mobile keeps all six on one scroll, desktop walks them one per step (1–3 required and gated, 4–6 optional and ungated; step 6 carries the Build submit) |
...
### 35.7 `travel-guide.js` — prototype-only hooks

`[data-tg-q]` (+ `data-tg-required`, `data-tg-max`, `data-tg-single`) · `[data-tg-generate]`
· `[data-tg-continue]` + `[data-tg-goto]` · `[data-tg-hint]` · `[data-tg-count]` ·
`[data-tg-seg]` · `[data-tg-cap]` · `[data-tg-build]` + `[data-tg-steps]` +
`[data-tg-elapsed]` · `[data-tg-add]` · `[data-tg-copy]` + `[data-tg-copy-label]` ·
`[data-tg-toast]` + `[data-tg-toast-el]` · `[data-tg-scroll]` · `[data-tg-live]`. Screen
activation is observed with a `MutationObserver`, so it composes with `flow.js`; the
build ladder is a fixed schedule (`Math.random` / `Date.now` free) so two runs look
identical.

**The form is the frame, not the screen.** Answers are counted across `.phone` /
`.browser` rather than the active screen, and every copy of `[data-tg-count]` /
`[data-tg-seg]` is written — so the "N of 3 answered" line and the six-segment meter
stay true whether the questions are stacked on one scroll (mobile) or walked one per
step (desktop). `[data-tg-continue]` is the step's own gate: it carries `data-goto`
only while that step's `[data-tg-required]` question is answered, so a required step
cannot be walked past, and arriving at step 6 is itself proof the three required
answers exist. The running "N still needed" hint is written only on the screen that
carries the submit; a step's hint is about that step and the script leaves it alone.
