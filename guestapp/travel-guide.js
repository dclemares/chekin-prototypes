/* AI Travel Guide — prototype-only interactions. No business logic, no network.

   [data-tg-q]            a question fieldset; [data-tg-required], [data-tg-min="n"],
                          [data-tg-max="n"], [data-tg-single]
   ...
   [data-tg-continue]     a step's Continue (desktop, one question per screen) —
                          gated by that step's own [data-tg-required] question;
                          [data-tg-goto] names the next step. Counting is
                          frame-scoped, so the meter and "N of 3" stay true
                          whether the questions are stacked or stepped.
...
(function () {
  ...
  /* ============================================================
     ...
     ============================================================ */
  var REQUIRED = ['pace', 'interests', 'budget'];

  function answersOf(screen, id) {
    return all(screen, '[data-tg-q="' + id + '"] input:checked');
  }

  /* A question can ask for more than one answer (interests wants two), so
     "answered" is its own minimum, not simply "not empty". */
  function minOf(frame, id) {
    var q = frame.querySelector('[data-tg-q="' + id + '"]');
    var min = q && q.getAttribute('data-tg-min');
    return min ? parseInt(min, 10) : 1;
  }

  function isAnswered(frame, id) {
    return answersOf(frame, id).length >= minOf(frame, id);
  }

  /* The form is the frame, not the screen. Mobile keeps all four questions on
     one screen, desktop walks them one per step — so "N of 3 answered" and the
     four-segment meter are only true if they are counted across the whole frame
     and written to every copy of them. */
  function syncQuestions(node) {
    var frame = frameOf(node);
    var screen = node.closest ? (node.closest('.flow-screen') || frame) : frame;
    var answeredRequired = REQUIRED.filter(function (id) { return isAnswered(frame, id); });

    /* Meter: a segment lights for the question it belongs to — not for the
       Nth answer given, as the shipped `answeredCount` bar does. */
    all(frame, '[data-tg-seg]').forEach(function (seg) {
      var id = seg.getAttribute('data-tg-seg');
      seg.classList.toggle('on', isAnswered(frame, id));
    });

    all(frame, '[data-tg-count]').forEach(function (counter) {
    ...
    all(frame, '[data-tg-max]').forEach(function (q) {
    ...
    all(frame, '[data-tg-q]').forEach(function (q) {
      if (all(q, 'input:checked').length) q.classList.remove('is-error');
    });

    /* Per-step Continue (desktop): gated by that step's own required question,
       so a required step cannot be walked past — which is the whole reason the
       questions are stepped rather than stacked. Reaching the last step is
       therefore proof the three required answers exist, and the Build button
       there needs no gate of its own. */
    all(frame, '[data-tg-continue]').forEach(function (btn) {
      var step = btn.closest('.flow-screen');
      var q = step && step.querySelector('[data-tg-q][data-tg-required]');
      var ok = !q || isAnswered(frame, q.getAttribute('data-tg-q'));
      if (ok) btn.setAttribute('data-goto', btn.getAttribute('data-tg-goto'));
      else btn.removeAttribute('data-goto');
      var stepHint = step && step.querySelector('[data-tg-hint]');
      if (stepHint) stepHint.classList.toggle('is-ready', ok);
    });
    ...
    /* Only the screen that carries the submit gets the running "N still
       needed" hint — a step screen's hint is about that step, and the JS must
       not overwrite it. */
    var missing = 3 - answeredRequired.length;
    var hint = btn && screen.querySelector('[data-tg-hint] span');
    if (hint) {
    ...
    if (ready && !screen.hasAttribute('data-tg-announced')) {
  ...
  function flagGaps(screen) {
    ...
    REQUIRED.forEach(function (id) {
      ...
      if (!q) return;
      var empty = !isAnswered(screen, id);
  ...
  document.addEventListener('click', function (e) {
    ...
    /* Generate / step Continue — an unsatisfied press is a teaching moment,
       not a no-op. */
    if ((el = e.target.closest('[data-tg-generate],[data-tg-continue]'))) {
